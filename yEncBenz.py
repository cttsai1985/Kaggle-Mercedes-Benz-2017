...
if __name__ == '__main__':
    ...
    df_train['Xid'] = df_train[target_id].apply(log1p) #may be useful to handle the duplicates
    #test
    ...
    l1r = 0.25
    eps = 0.05
    glm = SGDRegressor(loss='huber', penalty='elasticnet', alpha=a, l1_ratio=l1r, fit_intercept=True, n_iter=100, shuffle=True, verbose=0, epsilon=eps, random_state=seed_val, learning_rate='invscaling', eta0=0.01, power_t=0.25, warm_start=False, average=False)
    #extratree
    trees, leafs, depth = 1600, 128, 12
    xt = ExtraTreesRegressor(n_estimators=trees, criterion='mse', max_depth=depth, min_samples_split=4, min_samples_leaf=4, min_weight_fraction_leaf=0.0, max_features='auto', max_leaf_nodes=leafs, bootstrap=True, oob_score=True, n_jobs=-1, random_state=seed_val, verbose=0, warm_start=False)
    ...
    for nr_fold in range(nr_splits):
        ...
        print('XT r2={:.6f}, rmse={:.2f}'.format(r2_score(y_valid, preds), rmse), flush=True)
        ...
        preds = reg.predict(X_valid)
        rmse = sqrt(mean_squared_error(y_valid, preds))
        print('SDGR r2={:.6f}, rmse={:.2f}'.format(r2_score(y_valid, preds), rmse), flush=True)      
        collect_predict(preds, sub_train, d=nr_splits, col=target_this, opt_test=False)
        collect_predict(reg.predict(X_test), test_preds, d=nr_splits, col=target_this, opt_test=True)
    ...
    #find best weights
    f_preds = [f for f in train_preds.columns.tolist() if f.startswith('y_')]
    print('optimizing weights')
    trials, best, best_rmse = 1000, 0, 0
    best_w = []
    for i in range(trials):
        ...
        if score > best:
            best = score
            best_rmse = sqrt(mean_squared_error(raw_train_y, df_train['tmp'].apply(lambda x: x/s)))
            best_w = [w / s for w in weights]
            print('no {:04d} r2 = {:.9f}, rmse={:.2f}, current best ({})'.format(i, best, best_rmse, best_w), flush=True)
    ...
    sub.to_csv("{}_wsum_s{:.6f}_rmse{:.2f}.csv".format(tmstmp, best, best_rmse), index=False)
